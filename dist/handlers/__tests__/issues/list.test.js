...
import { mockResponse, mockErrorResponse, mockNetworkError } from "../../../lib/__tests__/helpers/mocks.js";
...
describe('list_issues', () => {
    ...
    describe('MCP Response Format', () => {
        it('returns valid MCP response for successful fetch', async () => {
            // Arrange
            mockFetch.mockImplementationOnce(() => mockResponse(fixtures.issueListResponse));
            // Act
        ...
        it('returns valid MCP error response for API error', async () => {
            ...
            mockFetch.mockImplementationOnce(() => mockErrorResponse(400, [errorMessage]));
            // Act
            const response = await handlers.list_issues({});
            // Assert
            assertMcpToolResponse(response);
    ...
    describe('Response Content', () => {
        ...
        it('escapes special XML characters in text fields', async () => {
            ...
            mockFetch.mockImplementationOnce(() => mockResponse(issueWithSpecialChars));
            // Act
            const response = await handlers.list_issues({});
            // Assert
            assertMcpToolResponse(response);
    ...
    describe('Error Handling', () => {
        ...
        it('handles API errors correctly', async () => {
            // Arrange
            mockFetch.mockImplementationOnce(() => mockErrorResponse(500, ["Internal server error"]));
            // Act
            const response = await handlers.list_issues({});
            // Assert
            assertMcpToolResponse(response);
        ...
        it('handles rate limiting correctly', async () => {
            // Arrange
            mockFetch.mockImplementationOnce(() => mockErrorResponse(429, ["Too many requests"]));
            // Act
            const response = await handlers.list_issues({});
            // Assert
            assertMcpToolResponse(response);
