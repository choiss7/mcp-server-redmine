...
export function formatUser(user) {
    ...
    if (user.login)
        fields.set('login', user.login);
    if (user.mail)
        fields.set('mail', user.mail);
    if (user.api_key)
        fields.set('api_key', user.api_key);
    if (user.status !== undefined)
        fields.set('status', formatStatus(user.status));
    if (user.last_login_on)
        fields.set('last_login_on', user.last_login_on);
    if (user.admin)
        fields.set('admin', 'true');
    if (user.updated_on)
        fields.set('updated_on', user.updated_on);
    if (user.passwd_changed_on)
        fields.set('passwd_changed_on', user.passwd_changed_on);
    ...
    if ((_a = user.custom_fields) === null || _a === void 0 ? void 0 : _a.length) {
    ...
    if ((_b = user.memberships) === null || _b === void 0 ? void 0 : _b.length) {
    ...
    if ((_c = user.groups) === null || _c === void 0 ? void 0 : _c.length) {
 ...
 */
export function formatUsers(response) {
 ...
 */
export function formatUserResult(user, action) {
    return `<?xml version="1.0" encoding="UTF-8"?>
<response>
 ...
 */
export function formatUserDeleted(id) {
