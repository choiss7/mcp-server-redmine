...
import { IssueQuerySchema, RedmineIssueSchema, validateListIssueIncludes, validateShowIssueIncludes, } from "../types/issues/schema.js";
export class IssuesClient extends BaseClient {
    ...
    async getIssues(params) {
        ...
        if ((params === null || params === void 0 ? void 0 : params.include) && !validateListIssueIncludes(params.include)) {
        ...
        return await this.performRequest(`issues.json${query ? `?${query}` : ""}`);
    }
    /**
    ...
    async getIssue(id, params) {
        // Validate include parameter
        if ((params === null || params === void 0 ? void 0 : params.include) && !validateShowIssueIncludes(params.include)) {
        ...
        const response = await this.performRequest(`issues/${id}.json${query ? `?${query}` : ""}`);
        return {
    ...
    async createIssue(issue) {
        const response = await this.performRequest("issues.json", {
            method: "POST",
    ...
    async updateIssue(id, issue) {
        const response = await this.performRequest(`issues/${id}.json`, {
            method: "PUT",
            body: JSON.stringify({ issue }),
        });
        return {
