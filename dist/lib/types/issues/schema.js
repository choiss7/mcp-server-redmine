import { z } from "zod";
// バリデーション関数
export function validateListIssueIncludes(include) {
    const LIST_ISSUE_INCLUDES = ['attachments', 'relations'];
    const includes = include.split(',');
    return includes.every(inc => LIST_ISSUE_INCLUDES.includes(inc));
}
export function validateShowIssueIncludes(include) {
    ...
    const includes = include.split(',');
    return includes.every(inc => SHOW_ISSUE_INCLUDES.includes(inc));
}
// 基本的なクエリパラメータのスキーマ
