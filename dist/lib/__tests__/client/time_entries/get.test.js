...
describe("Time Entries API (GET)", () => {
    ...
    describe("GET /time_entries.json (getTimeEntries)", () => {
        it("fetches time entries without parameters", async () => {
            // Arrange
            mockFetch.mockImplementationOnce(async () => mockResponse(fixtures.timeEntryListResponse));
            // Act
        ...
        it("handles error", async () => {
            ...
            // Act & Assert
            await expect(client.getTimeEntries({ invalid_param: "value" }))
    ...
    describe("GET /time_entries/:id.json (getTimeEntry)", () => {
        ...
        it("fetches a single time entry", async () => {
            // Arrange
            mockFetch.mockImplementationOnce(async () => mockResponse(fixtures.singleTimeEntryResponse));
            // Act
            const result = await client.getTimeEntry(timeEntryId);
            // Assert
            const expectedUrl = new URL(`/time_entries/${timeEntryId}.json`, config.redmine.host);
            expect(mockFetch).toHaveBeenCalledWith(expectedUrl.toString(), expect.objectContaining({
                method: "GET",
                headers: expect.objectContaining({
                    Accept: "application/json",
        ...
        it("handles error", async () => {
            // Arrange
            mockFetch.mockImplementationOnce(async () => mockErrorResponse(404, ["Time entry not found"]));
            // Act & Assert
