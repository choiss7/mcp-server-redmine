...
describe("Projects API (GET)", () => {
    ...
    describe("GET /projects.json (getProjects)", () => {
        it("fetches projects without parameters", async () => {
            // Arrange
            mockFetch.mockImplementationOnce(async () => mockResponse(fixtures.projectListResponse));
            // Act
        ...
        describe("error handling", () => {
            it("handles invalid status value", async () => {
                // Arrange
                mockFetch.mockImplementationOnce(async () => mockErrorResponse(400, ["Invalid status value"]));
                // Act & Assert
                await expect(client.getProjects({ status: 999 })).rejects.toThrow(RedmineApiError);
            });
            it("handles invalid include parameter", async () => {
                ...
                mockFetch.mockImplementationOnce(async () => mockErrorResponse(400, ["Invalid include value"]));
                // Act & Assert
                await expect(client.getProjects(params)).rejects.toThrow(RedmineApiError);
            });
            it("handles server error", async () => {
                // Arrange
                mockFetch.mockImplementationOnce(async () => mockErrorResponse(500, ["Internal server error"]));
                // Act & Assert
                await expect(client.getProjects()).rejects.toThrow(RedmineApiError);
            });
            it("handles unauthorized access", async () => {
                // Arrange
                mockFetch.mockImplementationOnce(async () => mockErrorResponse(403, ["Unauthorized access"]));
                // Act & Assert
                await expect(client.getProjects()).rejects.toThrow(RedmineApiError);
            });
        });
    ...
    describe("GET /projects/:id.json (getProject)", () => {
        ...
        it("fetches a single project by ID", async () => {
            // Arrange
            mockFetch.mockImplementationOnce(async () => mockResponse(fixtures.singleProjectResponse));
            // Act
            const result = await client.getProject(projectId);
            // Assert
            const expectedUrl = new URL(`/projects/${projectId}.json`, config.redmine.host);
            expect(mockFetch).toHaveBeenCalledWith(expectedUrl.toString(), expect.objectContaining({
                method: "GET",
                headers: expect.objectContaining({
                    Accept: "application/json",
        ...
        it("fetches a single project by identifier", async () => {
            ...
            // Assert
            const expectedUrl = new URL(`/projects/${projectIdentifier}.json`, config.redmine.host);
            expect(mockFetch).toHaveBeenCalledWith(expectedUrl.toString(), expect.objectContaining({
                method: "GET",
                headers: expect.objectContaining({
                    Accept: "application/json",
        ...
        describe("including associated data", () => {
            it("includes single type of associated data", async () => {
                ...
                mockFetch.mockImplementationOnce(async () => mockResponse(fixtures.singleProjectWithIncludesResponse));
                // Act
        ...
        describe("error handling", () => {
            it("handles invalid ID format", async () => {
                // Arrange
                mockFetch.mockImplementationOnce(async () => mockErrorResponse(404, ["Project not found"]));
                // Act & Assert
                await expect(client.getProject("invalid-id")).rejects.toThrow(RedmineApiError);
            });
            ...
            it("handles server error", async () => {
                ...
                mockFetch.mockImplementationOnce(async () => mockErrorResponse(500, ["Internal server error"]));
                // Act & Assert
                await expect(client.getProject(projectId)).rejects.toThrow(RedmineApiError);
            });
            it("handles unauthorized access", async () => {
                // Arrange
                mockFetch.mockImplementationOnce(async () => mockErrorResponse(403, ["Unauthorized access"]));
