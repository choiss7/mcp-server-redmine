...
describe("Users API (GET)", () => {
    ...
    describe("GET /users.json (getUsers)", () => {
        it("fetches users list without parameters", async () => {
            // Arrange
            mockFetch.mockImplementationOnce(async () => mockResponse(fixtures.userListResponse));
            // Act
        ...
        it("handles forbidden error (non-admin access)", async () => {
            // Arrange
            mockFetch.mockImplementationOnce(async () => mockErrorResponse(403, ["Forbidden. Admin privileges required."]));
            // Act & Assert
            await expect(client.getUsers()).rejects.toThrow("Redmine API error: 403 Error\nForbidden. Admin privileges required.");
        });
    });
    describe("GET /users/:id.json (getUser)", () => {
        it("fetches single user successfully", async () => {
            // Arrange
            mockFetch.mockImplementationOnce(async () => mockResponse(fixtures.singleUserResponse));
            const userId = fixtures.singleUserResponse.user.id;
            // Act
            const result = await client.getUser(userId);
            // Assert
            const expectedUrl = new URL(`/users/${userId}.json`, config.redmine.host);
            expect(mockFetch).toHaveBeenCalledWith(expectedUrl.toString(), expect.objectContaining({
                method: "GET",
                headers: expect.objectContaining({
                    Accept: "application/json",
        ...
        it("fetches current user successfully", async () => {
            ...
            // Assert
            const expectedUrl = new URL("/users/current.json", config.redmine.host);
            expect(mockFetch).toHaveBeenCalledWith(expectedUrl.toString(), expect.objectContaining({
                method: "GET",
                headers: expect.objectContaining({
                    Accept: "application/json",
        ...
        describe("including associated data", () => {
            it("includes single type of associated data", async () => {
                // Arrange
                mockFetch.mockImplementationOnce(async () => mockResponse(fixtures.singleUserWithIncludesResponse));
                // Act
        ...
        describe("error handling", () => {
            it("handles invalid include parameter", async () => {
                // Act & Assert
                await expect(client.getUser(1, { include: "invalid,values" })).rejects.toThrow("Invalid include parameter. Valid values are: memberships, groups");
            });
            it("handles user not found error", async () => {
                // Arrange
                mockFetch.mockImplementationOnce(async () => mockErrorResponse(404, ["User not found"]));
                // Act & Assert
